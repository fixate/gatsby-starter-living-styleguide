...
const reduceCats = (acc, cat) => {
  ...
  newAcc = cat.subCat
    ? newAcc.map(c => {
        const alreadyHasChild = (c.subCats || []).some(
          subCat => subCat.name === cat.subCat.name
        );
        c.subCats =
    ...
    <nav>
      {cats.map(cat => {
        ...
        return isIndex ? (
          ...
          <div key={cat.name}>
            {cat.name ? (
              <h4 style={{marginBottom: 0}}>{cat.name.replace('-', ' ')}</h4>
            ) : null}
            ...
            {cat.subCats.length
              ? cat.subCats.map(subCat => {
                  const subCatPages = catPages.filter(
                    p =>
                      p.relativeDirectory === [cat.name, subCat.name].join('/')
                  );
                  ...
                  return (
                    <div
                      key={subCat.name}
                      style={{marginBottom: '1em', marginLeft: '1em'}}>
                      <h4 style={{marginBottom: 0}}>{subCat.name}</h4>
